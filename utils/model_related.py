    ...
    def __init__(self, conf, resume):
        ...
        for key in modules_conf.keys():
            ...
            if not module_conf['freeze']:
                self.update_modules.append(module)

            # resuming training or init with specific initial model
            if resume:
                self.load(resume)
            else:
                initial_model_filename = module_conf.get('initial_model', None)
                if initial_model_filename:
                    initial_model_adaptor_class = module_conf.get('initial_model_adaptor', defaultModuleLoader)
